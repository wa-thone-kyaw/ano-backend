...
router.post("/attributes", async (req, res) => {
  ...
  try {
    ...
    if (pcsPerBox && pcsPerBox.length > 0) {
      const uniquePcsPerBox = [...new Set(pcsPerBox)]; // Remove duplicates

      // First, insert pcs_per_box values
      const pcsValues = uniquePcsPerBox.map((pcs) => [pcs]);
      ...
      const pcsPerBoxIds = pcsValues.map((pcs, index) => {
        return [product_id, pcsResult.insertId + index]; // Associating each product with each pcs_per_box value
      });
